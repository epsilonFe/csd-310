...
INSERT INTO wishlist(patron_id, book_id)
    VALUES (
        (SELECT patron_id FROM patrons WHERE first_name = 'John'),
        (SELECT book_id FROM book WHERE book_name = 'The Tangle Web')
    );

INSERT INTO wishlist(patron_id, book_id)
    VALUES (
        (SELECT patron_id FROM patrons WHERE first_name = 'Kevin'),
        (SELECT book_id FROM book WHERE book_name = 'Practical malware analysis')
    );

INSERT INTO wishlist(patron_id, book_id)
    VALUES (
        (SELECT patron_id FROM patrons WHERE first_name = 'Gary'),
        (SELECT book_id FROM book WHERE book_name = 'Adaptive security management architecture')
    );